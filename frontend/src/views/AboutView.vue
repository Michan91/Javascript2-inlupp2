<template>
  <div class="about">
    <h3 class="text-dark">Kontakta oss</h3>
    <div class="card w-100">
  <img src="https://sp-ao.shortpixel.ai/client/to_webp,q_glossy,ret_img,w_800,h_534/https://slowlivinghome.se/wp-content/uploads/2021/11/5280600221_5280600222_5280600223_5280602721_5280602722_5299001600_5280602723_Preview.jpg" class="card-img-top"/>
  <div class="card-body text-center mt-5">
    <!-- <h3 class="card-title mt-3 text-light">Mittljus</h3>
    <p class="card-text mt-3 text-light"></p> -->
  </div>
  <ul class="list-group list-group-flush text-center">
    <li class="list-group-item"><i class="fa-solid fa-phone me-2"></i>+46030303883</li>
    <li class="list-group-item"><i class="fa-solid fa-envelope me-2"></i>Dapibus ac facilisis in</li>
    <li class="list-group-item"><i class="fa-solid fa-location-dot me-2"></i>Vestibulum at eros</li>
    
  </ul>
  <div class="card-body text-center">
    <a href="#" class="card-link"><i class="fa-brands fa-instagram"></i></a>
    <a href="#" class="card-link"><i class="fa-brands fa-facebook-square"></i></a>
    <a href="#" class="card-link"><i class="fa-brands fa-linkedin"></i></a>
  </div>
</div>
  </div>
</template>

<style scoped>
.card-img-top {
position: absolute;
max-height: 450px;
object-fit: cover;
}
.card-body {
  position: relative;
}
.list-group {
  position: relative;
  margin-top: 20rem;
}
.fa-brands {
  font-size: 3rem;
  color: blue;
}
.fa-brands:hover {
  color: orange;
}
</style>
